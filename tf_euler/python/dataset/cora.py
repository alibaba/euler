...
cora_dir = os.path.join(current_dir, 'cora')


class cora(DataSet):

    def __init__(self, data_dir=cora_dir, data_type='all'):
        super(cora, self).__init__(data_dir, data_type)
        self.source_url = 'http://www.cs.umd.edu/~sen/lbc-proj/data/cora.tgz'
        self.origin_files = ['cora.cites',
                             'cora.content']
        self.max_node_id = 2708
        self.train_node_type = ['train']
        self.train_edge_type = ['train']
        self.total_size = 2708
        self.all_node_type = -1
        self.all_edge_type = ['train', 'train_removed']
        self.id_file = os.path.join(data_dir, 'cora_test.id')
        self.feature_idx = 'feature'
        self.feature_dim = 1433
        self.label_idx = 'label'
        self.label_dim = 7
        self.num_classes = 7
        self.test_start_num = 1708

    def download_data(self, source_url, out_dir):
        cora_tgz_dir = os.path.join(out_dir, 'cora.tgz')
