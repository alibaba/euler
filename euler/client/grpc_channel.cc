...
#include "euler/client/completion_queue_pool.h"
...
class GrpcClosure final : public GrpcCQTag {
 public:
  ...
  void OnCompleted(bool ok) override {
    Status status;
    if (!ok) {
      status = Status(ErrorCode::RPC_ERROR, "gRpc CQ not ok.");
    } else if (!ctx_->status.ok()) {
      status = Status(ErrorCode::RPC_ERROR,
      ...
      if (!ctx_->response->ParseFromZeroCopyStream(&reader)) {
        status = Status(ErrorCode::PROTO_ERROR, "Bad response.");
      }
    }
    ctx_->done(status);
    delete this;
...
GrpcChannel::GrpcChannel(const std::string &host_port,
    ...
    : RpcChannel(host_port),
      stub_(raw_channel),
      cq_(CompletionQueuePool::GetInstance()->NextCompletionQueue()) { }

void GrpcChannel::IssueRpcCall(RpcContext *ctx) {
  GrpcContext *grpc_ctx = dynamic_cast<GrpcContext *>(ctx);
  if (!grpc_ctx) {
    ctx->done(Status(ErrorCode::INVALID_ARGUMENT, "Wrong RpcContext."));
    return;
  }
