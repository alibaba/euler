...
TEST(FastWeightedCollectionTest, Init) {
  FastWeightedCollection<int32_t> fwc;
  ...
  for (size_t i = 0; i < 100000; ++i) {
    std::pair<int32_t, float> r = fwc.Sample();
    ++cnts[r.first];
  }
  ...
  std::cout << std::endl;
  // test iteration
  size_t size = fwc.GetSize();
  for (size_t i = 0; i < size; ++i) {
    std::pair<int32_t, float> id_weight_pair = fwc.Get(i);
