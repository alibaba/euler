...
bool FastEdge::DeSerialize(const char* s, size_t size) {
  ...
  if (!bytes_reader.GetInt32(&uint64_feature_type_num)) {
    LOG(ERROR) << "uint64 feature type num error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  if (!bytes_reader.GetInt32List(uint64_feature_type_num,
                                 &uint64_features_idx_)) {
    LOG(ERROR) << "uint64 feature idx list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  ...
  if (!bytes_reader.GetUInt64List(uint64_fv_num, &uint64_features_)) {
    LOG(ERROR) << "uint64 feature value list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  ...
  if (!bytes_reader.GetInt32(&float_feature_type_num)) {
    LOG(ERROR) << "float feature type num error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  if (!bytes_reader.GetInt32List(float_feature_type_num,
                                 &float_features_idx_)) {
    LOG(ERROR) << "float feature idx list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  ...
  if (!bytes_reader.GetFloatList(float_fv_num, &float_features_)) {
    LOG(ERROR) << "float feature value list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  ...
  if (!bytes_reader.GetInt32(&binary_feature_type_num)) {
    LOG(ERROR) << "binary feature type num error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  if (!bytes_reader.GetInt32List(binary_feature_type_num,
                                 &binary_features_idx_)) {
    LOG(ERROR) << "binary feature idx list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
  ...
  if (!bytes_reader.GetString(binary_fv_num, &binary_features_)) {
    LOG(ERROR) << "binary feature value list error, edge_node: " << src_id << "," << dst_id;
    return false;
  }
