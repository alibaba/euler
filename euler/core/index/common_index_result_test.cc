...
TEST(CommonIndexResultTest, Sample) {
  vector<pair<uint64_t, float>> data;
  data.push_back(std::make_pair(1, 1.0));
  data.push_back(std::make_pair(2, 2.0));
  data.push_back(std::make_pair(3, 1.0));
  CommonIndexResult cIR("common", data);
  {
    auto v = cIR.GetIds();
    ASSERT_EQ(v.size(), 3);
    ASSERT_EQ(v[0], 1);
    ASSERT_EQ(v[2], 3);
    auto f = cIR.SumWeight();
    ASSERT_EQ(f, 4);
  }
  {
    auto v = cIR.Sample(100000);
    vector<int32_t> cnts(4);
    for (auto& i : v) {
      cnts[i.first] += 1;
    }
  ...
  {
    auto c = cIR.ToCommonIndexResult();
    auto v = c->Sample(100000);
    vector<int32_t> cnts(4);
    for (auto& i : v) {
      cnts[i.first] += 1;
    }
...
TEST(CommonIndexResultTest, Intersection) {
  vector<pair<uint64_t, float>> data;
  data.push_back(std::make_pair(1, 1.0));
  data.push_back(std::make_pair(2, 2.0));
  data.push_back(std::make_pair(3, 1.0));
...

  {
    ...
    data.push_back(std::make_pair(1, 1.0));
    data.push_back(std::make_pair(2, 2.0));
    data.push_back(std::make_pair(4, 1.0));
    CommonIndexResult* c = new CommonIndexResult("common", data);
    std::shared_ptr<IndexResult> c_ptr(c);
    {
      auto cResult = cIR.Intersection(c_ptr);
      auto v = cResult->GetIds();
      ASSERT_EQ(v.size(), 2);
      ASSERT_EQ(v[0], 1);
      ASSERT_EQ(v[1], 2);
      auto f = cResult->SumWeight();
      ASSERT_EQ(f, 3);
    }
    {
      auto cResult = cIR.Intersection(c_ptr);
      auto v = cResult->Sample(100000);
  ...
  }
  {
    ...
    }
    {
      ...
      auto cResult = cIR.Intersection(h);
      auto v = cResult->GetIds();
      ASSERT_EQ(v.size(), 1);
      ASSERT_EQ(v[0], 2);
      auto f = cResult->SumWeight();
      ASSERT_EQ(f, 2);
      auto v2 = cResult->Sample(100000);
      vector<int32_t> cnts(5);
      for (auto& i : v2) {
        cnts[i.first] += 1;
      }
      ASSERT_EQ(cnts[2], 100000);
    }
  }
  {
    ...
    vector<float> weights = {3.0, 1.0, 2.0, 1.0, 2.0};
    rsIndex.Init(ids, values, weights);
    {
      auto r = rsIndex.Search(LESS, "3");
      auto cResult = cIR.Intersection(r);
      auto v = cResult->GetIds();
      ASSERT_EQ(v.size(), 2);
      ASSERT_EQ(v[0], 1);
      ASSERT_EQ(v[1], 2);
...
TEST(CommonIndexResultTest, Union) {
  vector<pair<uint64_t, float>> data;
  data.push_back(std::make_pair(1, 1.0));
  data.push_back(std::make_pair(2, 2.0));
  data.push_back(std::make_pair(3, 1.0));
  ...
  }
  {
    ...
    rsIndex.Init(ids, values, weights);
    {
      auto tmp = rsIndex.Search(LESS, "2");
      auto r = tmp->Union(rsIndex.Search(GREATER_EQ, "3"));
      auto cResult = cIR.Union(r);
      auto v = cResult->GetIds();
      ASSERT_EQ(v.size(), 5);
      ASSERT_EQ(v[0], 0);
      ASSERT_EQ(v[4], 4);
      ...
      ASSERT_TRUE(cnts[2]*1.0/cnts[1] > 1.9 && cnts[2]*1.0/cnts[1] < 2.1);
      ASSERT_TRUE(cnts[0]*1.0/cnts[1] > 2.9 && cnts[0]*1.0/cnts[1] < 3.1);
