...
class SampleNodeSplit: public OpKernel {
 public:
  explicit SampleNodeSplit(const std::string& name) : OpKernel(name) {}
  void Compute(const DAGNodeProto& node_def,
               OpKernelContext* ctx) override;
};

void SampleNodeSplit::Compute(const DAGNodeProto& node_def,
                              OpKernelContext* ctx) {
  // get split num
  ...
  // get node_type tensor
  Tensor* node_type_tensor = nullptr;
  ctx->tensor(node_def.inputs(1), &node_type_tensor);
  std::vector<int32_t> node_types(node_type_tensor->NumElements());
  std::copy(node_type_tensor->Raw<int32_t>(),
            node_type_tensor->Raw<int32_t>() + node_types.size(),
            node_types.begin());
  ...
  for (int32_t i = 0; i < split_num; ++i) {
    ...
    for (int32_t node_type : node_types) {
      if (node_type == -1) {
        node_type = shard_node_weight.size() - 1;
        if (node_types.size() > 1) {
          EULER_LOG(FATAL) << "ERROR node type!";
        }
      }
      sum_weight0 += shard_node_weight[node_type][i];
      sum_weight1 += shard_node_weight[node_type][split_num];
    }
    if (fabs(sum_weight1 - 0.0) < 0.0000001) {
      EULER_LOG(FATAL) << "node type sum weight is zero!";
    }
  ...
  for (int32_t i = 0; i < split_num; ++i) {
    ...
    *(merge_idx_t->Raw<int32_t>()) = begin_id;
    begin_id += split_cnt[i];
  }
}

REGISTER_OP_KERNEL("SAMPLE_NODE_SPLIT", SampleNodeSplit);
