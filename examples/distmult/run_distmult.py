...
def define_network_flags():
    ...
    tf.flags.DEFINE_integer('embedding_dim', 100, 'Embedding dimension.')
    ...
    tf.flags.DEFINE_bool('l2_regular', True, 'if use l2 regular')
    tf.flags.DEFINE_float('regular_param', 0.0001, 'l2 regular param')
    tf.flags.DEFINE_integer('num_epochs', 20, 'Epochs to train')
...
def main(_):
    ...
    #print("-----num_negs: {}".format(num_negs))
    ...
    model = DistMult(
        ...
        l2_regular=flags_obj.l2_regular,
        regular_param=flags_obj.regular_param)
