...
class GNN(BaseGNNNet):
    ...
    def get_conv(self, conv_class, dim):
        return conv_class(dim, self.mlp, self.eps, self.train_eps)
...
class GIN(GraphModel):
    ...
    def __init__(self, dims, metapath, label_dim,
                 feature_idx, feature_max_id,
                 mlp=None, eps=0.0, train_eps=False):
        super(GIN, self).__init__(label_dim)
        self.gnn = GNN('gin', 'full', dims, None, metapath,
                       feature_idx, feature_max_id,
                       mlp=mlp, eps=eps, train_eps=train_eps)
