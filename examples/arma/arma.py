...
class GNN(BaseGNNNet):
    ...
    def get_conv(self, conv_class, dim):
        return conv_class(dim, self.K, self.num_layers)


class ARMA(SuperviseModel):

    def __init__(self, dims, metapath,
                 feature_idx, feature_dim,
                 label_idx, label_dim,
                 K=2, num_layers=2):
        super(ARMA, self).__init__(label_idx,
                                   label_dim)
        self.gnn = GNN('arma', 'full', dims, None, metapath,
                       feature_idx, feature_dim, K, num_layers)
