...
class TransD(TransX):
    def __init__(self, node_type, edge_type,
                 ...
                 corrupt='both',
                 *args, **kwargs):
        super(TransD, self).__init__(node_type, edge_type,
                                     node_max_id, edge_max_id,
                                     ent_dim, rel_dim,
        ...
        if ent_dim != rel_dim:
            raise ValueError('Entity dim and Relation dim'
                             'should be equal in TransD')
        self.margin = margin
        self.entity_transfer = tf_euler.utils.layers.Embedding(node_max_id+1,
                                                               ent_dim)
        self.relation_transfer = tf_euler.utils.layers.Embedding(edge_max_id+1,
                                                                 rel_dim)

    def projection(self, embedding, ent_trans, rel_trans):
        output_shape = embedding.shape
        embedding = tf.reshape(embedding, [-1, self.ent_dim])
        ent_trans = tf.reshape(ent_trans, [-1, self.ent_dim])
        rel_trans = tf.reshape(rel_trans, [-1, self.ent_dim])
        project = tf.reduce_sum(embedding * ent_trans, 1,
                                keep_dims=True) * rel_trans
        embedding = tf.nn.l2_normalize(embedding + project, 1)
    ...
    def generate_embedding(self, src, dst, neg, rel):
        ...
        src_trans = self.entity_transfer(src)
        dst_trans = self.entity_transfer(dst)
        neg_trans = self.entity_transfer(neg)
        rel_trans = self.relation_transfer(rel)
        rel_trans_expand = tf.tile(rel_trans, [1, self.num_negs, 1])
        src_emb = self.projection(src_emb, src_trans, rel_trans)
        dst_emb = self.projection(dst_emb, dst_trans, rel_trans)
        neg_emb = self.projection(neg_emb, neg_trans, rel_trans_expand)
    ...
    def loss_fn(self, pos_scores, neg_scores):
        ...
        if self.corrupt == 'both':
            ...
            neg_scores_mean = tf.reduce_mean(
                                  tf.reshape(neg_scores, [-1, self.num_negs]),
                                  axis=-1,
                                  keep_dims=True)
