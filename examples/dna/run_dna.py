...
def define_network_flags():
    ...
    tf.flags.DEFINE_integer('layers', 1, 'GCN convolution layer number.')
    tf.flags.DEFINE_integer('group_num', 8, 'DNA group number.')
    tf.flags.DEFINE_integer('head_num', 1, 'DNA attention head number.')
    tf.flags.DEFINE_integer('batch_size', 32, 'Mini-batch size')
    tf.flags.DEFINE_integer('num_epochs', 15, 'Epochs to train')
