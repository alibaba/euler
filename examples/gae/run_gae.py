...
def main(_):
    ...
    if flags_obj.run_mode == 'train':
        ...
        model = GraphAutoEncoder(node_encoder, dims, fanouts,
                                 metapath, euler_graph.feature_idx,
                                 euler_graph.feature_dim, -1,
                                 ['train'], euler_graph.max_node_id, 10)
    else:
        metapath = [euler_graph.all_edge_type] * flags_obj.layers
        model = GraphAutoEncoder(node_encoder, dims, fanouts,
                                 metapath, euler_graph.feature_idx,
                                 euler_graph.feature_dim, -1,
                                 ['train_removed'], euler_graph.max_node_id, 10)
