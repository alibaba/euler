...
def define_network_flags():
    tf.flags.DEFINE_string('dataset', 'ppi', 'Dataset name.')
    ...
    tf.flags.DEFINE_integer('num_epochs', 30, 'Epochs to train')
    ...
    tf.flags.DEFINE_enum('optimizer', 'adam', ['adam', 'adagrad', 'sgd', 'momentum'],
                         'Optimizer algorithm')
    tf.flags.DEFINE_enum('metric', 'mrr', ['mrr', 'mr', 'hit10'],
                         'Metric name.')
...
def main(_):
    ...
    model = DGI(euler_graph.all_node_type,
                euler_graph.all_edge_type,
                euler_graph.max_node_id,
                metapath, fanouts, flags_obj.hidden_dim,
