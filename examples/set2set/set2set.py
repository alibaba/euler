...
class Set2SetModel(GraphModel):

    def __init__(self, dims, metapath, label_dim,
                 feature_idx, feature_max_id,
                 processing_steps=4,
                 lstm_layers=2):
        super(Set2SetModel, self).__init__(label_dim)
        self.gnn = GNN('sage', 'full', dims, None, metapath,
                       feature_idx, feature_max_id)
        self.pool = tf_euler.graph_pool.Set2SetPool(
            dims[-1],
